...
## 环境准备
- Java 21 (推荐通过 `winget install --id Microsoft.OpenJDK.21 --source winget` 安装)。
- Node.js 18 及以上版本（已验证 `v18.18.2` 可用）。
- Maven 与 npm 已随项目附带的 `mvnw` 和前端依赖满足，无需额外安装。
...
1. 打开 PowerShell，进入项目根目录：
	```powershell
	cd C:\Users\jsj31\Desktop\HKU\7606NLP\yu-ai-agent
	```
2. 为当前会话切换至 JDK21：
	```powershell
	...
	```
3. 启动 Spring Boot 应用（已关闭测试用例，加快启动速度）：
	```powershell
...
### RAG 功能开关
- 默认将 `hku.ai.rag.enabled` 设为 `false`，这样在没有有效 DashScope API 密钥时也能成功启动。
- 若需要启用 RAG，修改 `src/main/resources/application.yml`：
  ```yaml
  hku:
	 ai:
		rag:
		  enabled: true
  ```
- 同时把 `spring.ai.dashscope.api-key` 替换为有效密钥，并重启后端。
...
## 额外提示
- 如需永久生效的 `JAVA_HOME`，可在“系统属性 -> 环境变量”中设置同样的路径，并将 `%JAVA_HOME%\bin` 移到用户 Path 的首位。
- 每次修改配置后，建议重新执行对应的启动命令，确认控制台无异常告警再进行下一步操作。
- 若要同时运行前后端，可保持后端窗口不动，另开一个终端启动前端；需要全部结束时依次 `Ctrl+C` 退出两个进程。
